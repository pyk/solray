...
/// List deployable contracts in the given Foundry project.
///
...
mod tests {
    ...
    #[test]
    fn list_returns_deployable_contracts_only() {
        let contracts = list(fixture_path()).unwrap();

        assert_eq!(
            contracts,
            vec!["src/Counter.sol:Counter", "src/DataStore.sol:DataStore"]
        );
    }
}
